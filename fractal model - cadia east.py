...
range2 = np.arange(5*scale,1001*scale-0.1,5*scale)     
...
########################################### X direction Ascending=True#########################
list_var6 = []
for k in range(1340,1350,10):
    kriging_Xlist = []
    subdata1 = df_kriging[(pd.to_numeric(df_kriging["Z"], errors='coerce')>=k)& (pd.to_numeric(df_kriging["Z"], errors='coerce')<=k+10)]
    for i in range(12500,13000,10): #Y
        for j in range(k,k+11,5): #Z
...
for var6 in list_var6:
    ...
    axis.scatter(np.log10(range6),np.log10(var6),color='m',label='kriging(' + str(k) + '-' + str(k+10)+ ' from west to east' + ')' )
...
for k in range(1340,1350,10):
    ...
    for i in range(12500,13000,10): #Y
        for j in range(k,k+11,5): #Z
            subdata2 = subdata1[(pd.to_numeric(subdata1["Y"], errors='coerce')==i)& (pd.to_numeric(subdata1["Z"], errors='coerce')==j)]
            subdata2 = subdata2.sort_values(by='X',ascending=False)
            subdata2 = subdata2.reset_index(drop=True)
            # subdata1 = subdata['CU_kriging'].values
            kriging_Xlist.append(subdata2)
    var6 = []
...
for var6 in list_var6:
    ...
    axis.scatter(range3,var4,color='r')
    axis.scatter(range3,var5,color='b')
    axis.scatter(np.log10(range6),np.log10(var6),color='m',label='kriging(' + str(k) + '-' + str(k+10)+ ' from east to west' + ')' )
    # axis.scatter(np.log10(250),np.log10(np.var(onetruck_mean_list)),color='green',s=160,label='pseudo truck')
    axis.set_xlabel('log10(Tonnage)',fontsize=20)
    axis.set_ylabel('log10(Variance)',fontsize=20)
    axis.legend(loc='lower left',fontsize=20)

########################################### Y direction Ascending=True#########################
list_var6 = []
for k in range(1200,1350,10):
    kriging_Xlist = []
    subdata1 = df_kriging[(pd.to_numeric(df_kriging["Z"], errors='coerce')>=k)& (pd.to_numeric(df_kriging["Z"], errors='coerce')<=k+10)]
    for i in range(3000,4000,20): #X
        for j in range(k,k+11,5): #Z
            ...
            subdata2 = subdata2.sort_values(by='Y',ascending=True)
            subdata2 = subdata2.reset_index(drop=True)
            # subdata1 = subdata['CU_kriging'].values
            kriging_Xlist.append(subdata2)
    var6 = []
...
for var6 in list_var6:
    ...
    axis.scatter(range3,var4,color='r')
    axis.scatter(range3,var5,color='b')
    axis.plot(np.log10(range6),np.log10(var6),label='kriging')
    # axis.scatter(np.log10(250),np.log10(np.var(onetruck_mean_list)),color='green',s=160,label='pseudo truck')
    axis.set_xlabel('log10(Tonnage)',fontsize=20)
    axis.set_ylabel('log10(Variance)',fontsize=20)
    #axis.legend(loc='upper right',fontsize=28)

########################################### Y direction Ascending=False#########################
list_var6 = []
for k in range(1200,1350,10):
    kriging_Xlist = []
    subdata1 = df_kriging[(pd.to_numeric(df_kriging["Z"], errors='coerce')>=k)& (pd.to_numeric(df_kriging["Z"], errors='coerce')<=k+10)]
    for i in range(3000,4000,20): #X
        for j in range(k,k+11,5): #Z
            subdata2 = subdata1[(pd.to_numeric(subdata1["X"], errors='coerce')==i)& (pd.to_numeric(subdata1["Z"], errors='coerce')==j)]
            subdata2 = subdata2.sort_values(by='Y',ascending=False)
            subdata2 = subdata2.reset_index(drop=True)
            # subdata1 = subdata['CU_kriging'].values
            kriging_Xlist.append(subdata2)
    var6 = []
...
# fig, axis = plt.subplots(1,1,figsize=(12,8))
# axis.hist(mean6[0],bins=100,histtype='step',label = 'average 1 data',density=True)
# axis.hist(mean6[3],bins=100,histtype='step',label = 'average 4 data',density=True)
# axis.hist(mean6[6],bins=100,histtype='step',label = 'average 7 data',density=True)
# axis.hist(mean6[9],bins=100,histtype='step',label = 'average 10 data',density=True)
# axis.set_xlabel('metal grade',fontsize=24)
