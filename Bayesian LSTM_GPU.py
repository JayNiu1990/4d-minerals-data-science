...
df1 = pd.read_csv(r'C:\Users\NIU004\OneDrive - CSIRO\Desktop\Mineral sorting\Kansanshi\Kansanshi MRA Time3\2021-10-10.csv')
df2 = pd.read_csv(r'C:\Users\NIU004\OneDrive - CSIRO\Desktop\Mineral sorting\Kansanshi\Kansanshi MRA Time3\2021-10-11.csv')
df = pd.concat([df1,df2],axis=0,ignore_index=True)

df = df[(df['tonnage']>=2000) & (df['grade']>0)]
df['grade'] = np.log(df['grade'])
df = df.reset_index(drop=True)
df = df.drop(columns=['datetime','time','timestamp','tonnage','date'])
...
class BayesianLSTM(nn.Module):
    ...
    def __init__(self, n_features, output_length, batch_size):
        ...
        self.hidden_size_2 = 64#32 # number of decoder cells (from paper)
...
training_df['grade over 2000tonnage'] = inverse_transform(training_predictions.detach().cpu().numpy())
training_df['source'] = 'Training Prediction'

training_truth_df = pd.DataFrame()
training_truth_df['index'] = training_df['index']
training_truth_df['grade over 2000tonnage'] = df['grade'].iloc[offset:n_train + offset:1] 
...
testing_df['grade over 2000tonnage'] = inverse_transform(testing_predictions.detach().cpu().numpy())
testing_df['source'] = 'Test Prediction'

testing_truth_df = pd.DataFrame()
testing_truth_df['index'] = testing_df['index']
testing_truth_df['grade over 2000tonnage'] = df['grade'].iloc[n_train + offset::1] 
...
for i in range(n_experiments):
    ...
    test_uncertainty_df['grade_{}'.format(i)] = inverse_transform(experiment_predictions.detach().cpu().numpy())
...
test_uncertainty_plot_df = test_uncertainty_plot_df.loc[test_uncertainty_plot_df['index'].between(len(df)*0.8, len(df))]
truth_uncertainty_plot_df = testing_truth_df.copy(deep=True)
truth_uncertainty_plot_df = truth_uncertainty_plot_df.loc[testing_truth_df['index'].between(len(df)*0.8, len(df))]
...
real_trace = go.Scatter(
    ...
    y=truth_uncertainty_plot_df['grade over 2000tonnage'],
    mode='lines',
    fill=None,
    name='Real Values'
    )
...
fig.update_layout(title=dict(text='Uncertainty Quantification for MRA grade (w.t%) prediction vs Time Step',font=dict(size=30)),
                    xaxis_title='Time Index',
                    yaxis_title='Cu Grade (wt%)',
                    xaxis = dict(tickfont = dict(size=40),title_font=dict(size=40)),
                    yaxis = dict(tickfont = dict(size=40),title_font=dict(size=40)),
