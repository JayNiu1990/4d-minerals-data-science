if __name__ == '__main__':
    ...
    fig = px.scatter_3d(df[df['AL_ALT']==40], x="X",y="Y",z="Z",color="AL_ALT")
    ...
    MSE_MLE_outliers = []
    MSE_GMM_outliers = []
    for i,j in zip([2,43,60,79,95,145],np.arange(0,6)):
    ...
    MSE_MLE_outliers = []
    MSE_GMM_outliers = []
    #[32,48,56,85,122,131][56,56,56,56,56,56]
    ...
    MSE_MLE_outliers = []
    MSE_GMM_outliers = []
    for i,j in zip([56,56,56,56,56,56],np.arange(0,6)):
        df3= df2_new[df2_new['blocks']==i].sort_values(by=['CuT_dh_transfered'])
        X = np.array(df3['CuT_dh_transfered'])
        Y = np.array(df3['Fe_dh_transfered'])
        phi_x = np.vstack([X**0, X**1])
