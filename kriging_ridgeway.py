...
for line1 in lines1[1:]:
    ...
    list1.append(row)
df = pd.DataFrame(list1,columns=['SAMPLEID','HOLEID','PROJECTCODE','SAMPLEFROM','SAMPLETO','Au_ppm_BEST','Cu_ppm_BEST'])
df['Cu_ppm_BEST'] =df['Cu_ppm_BEST'].astype('float')
df['CU_wt'] = df['Cu_ppm_BEST']/10000

df = df[df['HOLEID'].isin(['UR291','UR292','UR331','UR332','UR333','UR341W2','UR364','UR365','UR510','UR511'])]
...
#df1 = df[0:200]
gridx = np.arange(x1,x2,30,dtype='float64')
gridy = np.arange(y1,y2,30,dtype='float64')
gridz = np.arange(z1,z2,10,dtype='float64')
zstar , ss = OK.execute('grid',gridx,gridy,gridz,backend="loop",n_closest_points=5)
...
xx1 = np.arange(x1, x2, 10).astype('float64')
yy1 = np.arange(y1, y2, 10).astype('float64')
zz1 = np.arange(z1, z2, 5).astype('float64')
...
xx2 = np.arange(3010, 3100, 10).astype('float64')
yy2 = np.arange(13010, 13100, 10).astype('float64')
zz2 = np.arange(1305, 1350, 5).astype('float64')
...
gridx = np.arange(x1,x2,10,dtype='float64')
gridy = np.arange(y1,y2,10,dtype='float64')
...
fig.update_traces(marker_size=6)
fig.update_layout(scene = dict(xaxis = dict(tick0=3000,dtick=10,tickmode='linear'),
                      yaxis = dict(tick0=12500,dtick=10,tickmode='linear'),
                      zaxis = dict(tick0=1300,dtick=10,tickmode='linear'))) 
fig.update_layout(scene_aspectmode='data')
fig.show()
