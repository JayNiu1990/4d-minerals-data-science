...
def gamv(
    ...
    for il in range(0, nlag + 2):
        ...
        vario[il] = vario[il]#0.5 * vario[il]
...
def variogram_loop(x, y, vr, xlag, xltol, nlag, azm, atol, bandwh):
    ...
    # The mathematical azimuth is measured counterclockwise from EW and
    # not clockwise from NS as the conventional azimuth is
    azmuth = (90.0 - azm) * math.pi / 180.0 #calculate azimuth in radian
    uvxazm = math.cos(azmuth)  #cos
    uvyazm = math.sin(azmuth)  #sin
    if atol <= 0.0:
        csatol = math.cos(45.0 * math.pi / 180.0)
    else:
        csatol = math.cos(atol * math.pi / 180.0)
                    ...
                    for ilag in range(1, nlag + 1):
                        ...
                        if (
                            ...
                            if lagbeg < 0:
                                lagbeg = ilag
                            lagend = ilag
                if lagend >= 0: ###lagend is the selected lag number for particular data pair
                    # Definition of the direction corresponding to the current
                    # pair. All directions are considered (overlapping of
                    # direction tolerance cones is allowed)

                    # Check for an acceptable azimuth angle
                    dxy = np.sqrt(max((dxs + dys), 0.0)) #distance of two data pair
                    if dxy < EPSLON:
                        dcazm = 1.0
                    else:
                        dcazm = (dx * uvxazm + dy * uvyazm) / dxy
...
def variogram_loop_3D(x, y, z, vr, xlag, xltol, nlag, azm, atol, dip, dtol, bandwh):
    """GSLIB's GAMV program (Deutsch and Journel, 1998) converted from the
    original Fortran to Python by Michael Pyrcz, the University of Texas at
    Austin (Jan, 2019).
    Note simplified for 3D, semivariogram only and one direction at a time.
    :param df: pandas DataFrame with the spatial data
    :param xcol: name of the x coordinate column
    :param ycol: name of the y coordinate column
    :param vcol: name of the property column
    ...
    nd = len(x)
    azmuth = (90.0 - azm) * math.pi / 180.0
    dip_angle = (90.0 - dip) * math.pi / 180.0
    uvxazm = math.cos(azmuth)
    uvyazm = math.sin(azmuth)
    
    uvydip = math.cos(dip_angle)
    uvzdip = math.sin(dip_angle)
    if atol <= 0.0:
        csatol = math.cos(45.0 * math.pi / 180.0) ##cos of aziumuth tolerance
    else:
        csatol = math.cos(atol * math.pi / 180.0) 
    if dtol <= 0.0:
        csdtol = math.cos(45.0 * math.pi / 180.0)  ##cos of dip tolerance
    else: 
        csdtol = math.cos(dtol * math.pi / 180.0)
                ...
                if h <= EPSLON:
                    ...
                    for ilag in range(1, nlag + 1):
                        if (
                            ...
                            lagend = ilag
                if lagend >= 0:
                    dxy = np.sqrt(max((dxs + dys), 0.0)) ###xy plane
                    dxz = np.sqrt(max((dxs + dzs), 0.0)) ###xz plane
                    dyz = np.sqrt(max((dys + dzs), 0.0)) ###yz plane
                    if dxy < EPSLON:
                        dcazm = 1.0
                        dcdip = 1.0
                    else:
                        ...
                        # The mathematical azimuth is measured counterclockwise from EW and
                        # not clockwise from NS as the conventional azimuth is
                        dcazm = (dx * uvxazm + dy * uvyazm) / dxy   #dcazm = (dx * uvxazm + dy * uvyazm) / dxy
                        #uvydip = math.cos(dip_angle)
                        #uvzdip = math.sin(dip_angle)
                        dcdip = (dxy * uvydip + dz * uvzdip) / h

                    # band = uvxazm * dy - uvyazm * dx
                    band_azm = uvxazm * dy - uvyazm * dx
                    band_dip = uvydip * dz - uvzdip * dxy

                    if (abs(dcazm) >= csatol) and (abs(band_azm) <= bandwh)and (abs(dcdip) >= csatol) and (abs(band_dip) <= bandwh):
                        omni = False
                        if atol >= 90.0:
                            omni = True
                        iv = 0
