if __name__ == '__main__':
    ...
    elevation_idxs1 = np.array(df2_new['elevation_inxs'])
    
    
    fig = px.scatter_3d(df2, x="X",y="Y",z="Z",color="BHID")
    fig.update_traces(marker_size=2)
    fig.update_layout(font=dict(size=10))
    fig.update_layout(scene_aspectmode='data')
    #fig.show()    
    ...
    with pm.Model() as pooled_model:
        ...
        Fe_mean = alpha + beta*df2['CuT_dh_log_noise'].values
        Fe = pm.Normal('Fe', mu = Fe_mean, sd = eps, observed = df2['Fe_dh_log_noise'])
    with pooled_model:
    ...
    with pm.Model() as unpooled_model:
        ...
        Fe_mean = alpha[elevation_idxs1] + beta[elevation_idxs1]*df2['CuT_dh_log_noise'].values
        Fe = pm.Normal('Fe', mu = Fe_mean, sd = eps,observed = df2['Fe_dh_log_noise'])
    ...
    with pm.Model() as hirearchical_model:
        ...
        Fe_mean = alpha[elevation_idxs1] + beta[elevation_idxs1]*df2['CuT_dh_log_noise'].values
        Fe = pm.Normal('Fe', mu = Fe_mean, sd = eps,observed = df2['Fe_dh_log_noise'], shape = n_elevation)
    ...
    plt.scatter(df2[df2['elevation_inxs']==n]['CuT_dh_log_noise'], df2[df2['elevation_inxs']==n]['Fe_dh_log_noise'],label='bore core (training)',s=200,color='k',marker = '.')
    plt.legend(fontsize=14,loc='upper left')
    plt.xlabel('log Cu')
    plt.ylabel('log Fe')
    plt.ylim(-5,5)
    plt.title('Elevation:' + str(df2[df2['elevation_inxs']==n]['elevation'].unique()[0]))
    plt.show()
    ...
    plt.scatter(df2[df2['elevation_inxs']==1]['CuT_dh_log_noise'], df2[df2['elevation_inxs']==1]['Fe_dh_log_noise'])
    ...
    df_label = [val for val in list(df2.elevation.unique()) for _ in (0, 1)]


    selection2 = list(df2.elevation.unique())[85:88] 
    ...
    for i,c in enumerate(selection2):
        #if i%2==1:
        c_data = df2.loc[df2.elevation==c]
        c_data = c_data.reset_index(drop=True)
        c_index = np.where(elevation==c)[0][0]
    ...
    df_label = [val for val in list(df2.elevation.unique()) for _ in (0, 1)]
    df_label1 = [val for val in df_label for _ in (0, 1)]
    df_label1 = pd.DataFrame(df_label1,columns=['elevation'])
    df_label1_drop = df_label1.drop_duplicates(keep='first')
    df_label2 = df_label1.drop(df_label1_drop.index)['elevation'].to_list()
    
    ###################plot complete/no/partial pooling regression on training/testing data####################
    selection3 = df_label2[255:264] #list(df2_test.elevation.unique())[1:4] #['2000-2100','2100-2200', '2200-2300'] 
    ...
    for i,c in enumerate(selection3):
        ...
        if (i-1)%3 ==1:#258
            ...
            axis[i].set_title('elevation:' + str(c))
            num=1
            for a_val, b_val in zip(hirearchical_trace['alpha'][:,c_index],hirearchical_trace['beta'][:,c_index]):
                if num==1:
            ...
            num=1
        #axis[i].plot(xvals,hirearchical_trace['alpha'][1000:,c_index].mean()+hirearchical_trace['beta'][1000:,c_index].mean()*xvals,'r',alpha=1,lw=1.,label='hirearchical')
            for a_val, b_val in zip(unpooled_trace['alpha'][:,c_index],unpooled_trace['beta'][:,c_index]):  
                if num==1:
            ...
            axis[i].set_title('elevation:' + str(c))
            num=1
            for a_val, b_val in zip(pooled_trace['alpha'][:],pooled_trace['beta'][:]):  
                if num==1:
